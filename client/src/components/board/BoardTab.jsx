...
export default function BoardTab({ board }) {
  ...
  const [posts, setPosts] = usePosts(board.board_id, auth)
    ...
    if (title === "notFav") {
      axios.post("/fav", { board_id: board.board_id, user_id: user.user_id }, {
        ...
        .then(res => {
          ...
          setUser(prevState => ({
            ...prevState,
            favs: [...prevState.favs, res.data]
          }))
        })
        .catch(err => console.log(err))

    } else if (title === "fav") {
      axios.delete(`/fav/${board_id}`)
        .then(res => {
            setUser(prevState => ({
            ...prevState,
  ...
  const content = board ? (
    <li className='h-40 border-2 bg-white border-sky-700 p-5 grid grid-cols-8 grid-rows-6'>
      ...
      <div className='col-start-6 col-end-9 row-start-6 row-end-7 h-8 w-full flex justify-between'>


        {user.favs.includes(board.board_id) ? (
          <button title='notFav' value={{board_id: board.board_id}} className='h-full p-2  cursor-pointer hover:scale-95' onClick={addFav}>
            <img src={notFavIcon} className='h-full' title="notFav" onClick={addFav} />
          </button>
        ) : (
          <button title='fav' className='h-full p-1 cursor-pointer hover:scale-95' onClick={addFav}>
            <img src={favIcon} className='h-full' onClick={addFav} title='fav' />
          </button>
        )}
        <Link className='h-full hover:scale-95' to={`/post/${board.board_id}`}>
